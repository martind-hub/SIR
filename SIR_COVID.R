...
# extract a zoomed in figure of the data
# plot the results for the infectious individuals

# set the observation times
time_segment <- c(500, 510, 520, 530, 540)
# extract the true data at the specified times
true_data <- output$infectious[time_segment]
# extract the observations
noisy_data <- noise[time_segment/10]
# extract the differences
diff_data <- noisy_data - true_data
# set the new true data
new_true_data <- c(true_data[1])
for (i in 1:length(time_segment)-1) {
  new_true_data <- c(new_true_data, new_true_data[i]+500*i)
}
# set the observed data
new_observed_data <- new_true_data + diff_data

# store the results in a data frame
final <- data.frame("time" = time_segment, "true" = new_true_data, "observed" = new_observed_data)

# plot the altered graph
ggplot(data = final, aes(x=time)) +
  geom_line(mapping = aes(y=true), colour = "red") +
  geom_point(mapping = aes(y=observed), colour = "blue", shape = 4, size = 2) +
  labs(x = "Time segmet", y = "Adjusted observations")
...
# extract the index at which the maximum for the infectious individuals occurs
max_index <- which.max(output$infectious)

# extract the number of susceptible individuals
S_Imax <- output$susceptible[max_index]
